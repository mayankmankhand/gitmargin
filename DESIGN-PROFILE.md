...
- **Hairline, improve run on the shared-mode panel** (issue #15, 2026-09-18) - the same seed, no new direction - critic 5/10 then 5/10 over the two rounds an improve run allows - **kept** (round 2). The run added authors, replies, a status badge, a name prompt and a Version line to the existing panel. Fixed from round 1: the Version list repeated the page's own version as a row, so it now lists only other versions and is a plain line when there are none. Everything else the critic raised was the standing panel brief below, not this cycle's. Receipt: the Design run line in the Outcomes of `plans/PLAN-issue-15.md`; screenshots were staged with a throwaway script because the toolkit's browser script cannot open a local file.

- **Hairline, improve run on the sign-in identity line** (issue #18, 2026-09-21) - the same seed, no new direction - critic 5/10 then 5/10 over the two rounds an improve run allows - **kept** (round 2). The run replaced the typed-name row with one text-only identity line and its states (signed out, waiting with a short code, blocked, not a member, failed, signed in), plus a verified author on cards drawn one step stronger. Fixed from round 1: Sign out moved inline to the right of the row; round 2 caught a regression that fix made (the provider name wrapped to its own line), fixed after the loop with one row and an ellipsis. Everything else the critic raised was the standing panel brief below. The review that followed changed the copy of the refusal state (verdict first) and added the GitLab handle after the name, both outside the critic loop. Receipt: the Step 7 Outcomes of `plans/PLAN-issue-18.md`; the review report `reports/review-orchestrator-2026-09-22-000006.md`, R1 to R5.
...
- **The panel, as a brief of its own** (from the issue #10 improve run, 2026-09-09). The critic's six gaps are a ready-made brief for a panel cycle rather than a failed one: anchor the composer to the panel's bottom, make the whole-thing note read as a field, give the comment-mode switch the weight Send has, pick one border treatment for name field, select and button, open up the type scale, cut the empty state to one instruction, and give the collapse chevron a hit area. Same seed. The owner said no design changes in the #10 cycle, so this waits for a cycle that is about the panel. The issue #15 improve run (2026-09-18) drew the same six again, twice, and added one idea worth keeping: collapse the always-open name field to a single line ("Commenting as Priya", click to edit), which also removes one of the three strips stacked above the first comment on a shared page. The issue #18 improve run (2026-09-21) added two more: the identity is stated twice on a signed-in page ("Commenting as Priya Shah" in the line, and "(you)" on every card of hers), and the owner's own remark from the live walk, "it is hard to track where the comment has been made", which is a wayfinding gap between the panel's list and the pins on the page.
