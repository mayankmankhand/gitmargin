...
| `tests/check.test.js` | node:test | `gitmargin check` (issue #16): each rule fires on its trigger and stays silent without it, the channel filter, the JSON output. `tests/cli-service.test.js` also covers `attach --require-trusted` refusing an address this machine has not used, with nothing reaching the service |
