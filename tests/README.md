...
| `tests/redraw.spec.js` | Playwright | a prototype that draws every step into the same area (issue #34), on `fixtures/redraw.html` in one redraw habit at a time (`window.redrawShape`: new markup or the same nodes with new words, the title in or apart from the card, a footer every step shares, tagged steps, a dialog step): a comment's pin stays on its step, while the logo, the help line and a shared footer keep theirs; each rule in `anchor.js` and `screen.js` has a test that fails when that rule alone is broken |
