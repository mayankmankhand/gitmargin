...
- **A new author** has no permission rules. **An author who followed the guide** has `Bash(gitmargin *)` in the temp project's `.claude/settings.local.json` (the guide stopped suggesting a global rule for `gitmargin-publish` in #30; the share skill carries its own): allow rules in `.claude/settings.json` are ignored until the folder is trusted.
