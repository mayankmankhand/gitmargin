...
- [ ] **v0 part 2, the rest (parked).** A Slack mirror, an MCP server, the phone mode, and the four one-day spikes ([#2](https://github.com/mayankmankhand/gitmargin/issues/2)) that sign-in inside a company must pass first: sign-in inside Slack's in-app browser, GitLab consent behaviour, publish timing on GitLab Pages (a first answer from [#37](https://github.com/mayankmankhand/gitmargin/issues/37)'s walk: 31 to 37 seconds from push to page), and corporate MFA policies inside that browser. Parked until there is a gitlab.com group, a Slack workspace, and phones to test with (section 7 of the split doc). Safari ([#7](https://github.com/mayankmankhand/gitmargin/issues/7)) and overlay polish ([#8](https://github.com/mayankmankhand/gitmargin/issues/8)) wait alongside.
